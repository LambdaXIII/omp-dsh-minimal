...
**关键约束（ablation 实测，`~/.agents/journal/research/2026-08-18-deepseek-anchor-text-sensitivity.md`）**：工具文本提及（消息里出现工具名/描述/schema，即使不可调用）破坏 `We need` 并诱发模型直接调用不可调用工具——极简环境必须**零工具文本**。因此本插件不注入工具 schema、不注入工具提及。
